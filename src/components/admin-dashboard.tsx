...
export default function AdminDashboard({ type = 'donations', user }: AdminDashboardProps) {
  ...
  useEffect(() => {
    console.log('AdminDashboard useEffect called with type:', type);
    if (user) {
      console.log('User is available, fetching data...');
      if (type === 'donations' || type === 'inventory') {
        fetchDonations();
      }
      if (type === 'requests' || type === 'inventory') {
        fetchRequests();
      }
      if (type === 'inventory') {
        fetchInventory();
      ...
      console.log('No user available in AdminDashboard');
    }
  }, [user, type]);
  ...
  const fetchDonations = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('Donations set:', data.donations);
      } else {
